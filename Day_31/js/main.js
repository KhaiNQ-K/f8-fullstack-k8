...
const app = {
  ...
  handleEvents: function () {
    ...
    document.onmousemove = function (e) {
      if (_this.isDrag) {
        _this.moveSpace =
          e.clientX - _this.initialClientX + _this.lastMoveSpace;
  ...
  loadLyricByTime() {
    ...
    const wordsList = lyric.data.sentences
      .filter((x) => x.words)
      .filter((x) =>
        x.words.some((y) => y.startTime <= time && y.endTime >= time)
      );
    var indexOfWordList = wordsList.map((x) =>
      x.words.findIndex((y) => y.startTime <= time && y.endTime >= time)
    );
    var indexOfWords = indexOfWordList.filter((x) => x !== -1);
    console.log(indexOfWordList);
    console.log(indexOfWords);
    // lyric.data.sentences.filter(x => x.words)
  },

  convertTime(time) {
    // const minus = Math.floor(time / 60);
    // const second = Math.floor(time - minus * 60);
    // console.log(minus * 1000 + second * 1000);
    return time * 1000;
    // return time * 1000;
  },
  loadSongInfo() {
